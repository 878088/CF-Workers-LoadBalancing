...
async function 建立传输管道(WS接口, TCP接口, 写入初始数据, TCP缓存 = [], WS缓存 = []) {
  ...
  Promise.allSettled(Array.from({ length: TCP并发数 }, () =>
...
return `
Vless：https${符号}${hostName}/${哎呀呀这是我的ID啊}/${转码}${转码2}
Clach：https${符号}${hostName}/${哎呀呀这是我的ID啊}/${小猫}${咪}
`;
}
...
if (私钥开关) {
  ...
  return 我的优选.map(获取优选 => {
    ...
    return `${转码}${转码2}${符号}${哎呀呀这是我的VL密钥}@${地址}:${端口}?encryption=none&${TLS开关}&sni=${hostName}&alpn=h3&fp=ios&type=ws&host=${hostName}&path=%2F%3Fed%3D2560#${节点名字}`;
  }).join("\n");
}
}
...
- GEOSITE,netflix,✈️ 起飞
- GEOIP,NETFLIX,✈️ 起飞,no-resolve
- GEOSITE,telegram,✈️ 起飞
